...
public sealed class ForgotPasswordCommandHandler(IUserRepostiory userRepostiory, IUnitOfWork unitOfWork, IMailService mailService) : IRequestHandler<ForgotPasswordCommand, Result<string>>
{
    public async Task<Result<string>> Handle(ForgotPasswordCommand request, CancellationToken cancellationToken)
    {
        User user = await userRepostiory.FirstOrDefaultAsync(x => x.Email.Value == request.Email);
...
<body>
    ...
    <table role=""presentation"" cellspacing=""0"" cellpadding=""0"" border=""0"" width=""100%"" style=""background-color: #f9f9f9;"">
        ...
        body = body.Replace("{UserName}", user.UserName.Value)
                   .Replace("{ResetPasswordUrl}", $"http://localhost:4200/reset-password/{user.ForgotPasswordCode!.Value}");

        await mailService.SendAsync(to, subject, body, cancellationToken);

        return Result<string>.Succeed("Şifre sıfırlama mailiniz gönderilmiştir. Lütfen mail adresinizi kontrol edin");
    }
}